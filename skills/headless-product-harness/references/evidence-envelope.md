...
```json
{
  ...
  "evidence": {
    "profile_loaded": true,
    "normalized_records_created": true
  },
  "claims": {
...
manual, mark it as manual evidence and keep the claim boundary explicit.

## Negative Claims

Include negative claims whenever adjacent surfaces are easy to overclaim:
...
Negative claims are not decoration. Emit a negative claim only when the command
...
A negative claim limits what later agents may inherit from the evidence. It does
not prove product failure; it only says the current command did not prove that
surface.

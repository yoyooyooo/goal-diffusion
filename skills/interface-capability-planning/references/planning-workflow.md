...
Do not place the full Interface Capability DSL inside `charter.yaml`; the Goal
...
```text
Product intent -> User task -> Interface Capability -> Surface / IA
...
```text
Headless Capability -> User Task -> Interface Capability -> Surface
-> Interaction Contract -> State Cases -> UI Harness -> Evidence
```

Preserve headless claim ceilings. A headless proof can prove facts; it does not

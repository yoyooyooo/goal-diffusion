...
3. Choose the smallest honest harness level:
   ...
   - `browser_visible`
   - `production_near`
4. State the claim ceiling before writing or running tests.
5. Pair UI proof with headless proof when the claim depends on product facts.
6. Emit evidence with positive tokens, negative claims, screenshots/logs where
...
render_wiring      -> controls, roles, pending/error, dispatch wiring proof
...
negative claims: `business_fact_claim=false` unless paired with headless proof,
...
when a durable record is needed. Include `interface_capability`, `scenario`,

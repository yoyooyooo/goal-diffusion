...
description: >-
  ...
  deciding which docs layer should hold Goal Diffusion artifacts, specs,
...
and skill handoffs; do not own product truth, a single Goal Pack charter, or
Goal Diffusion method-internal lifecycle. Initialization, migration, and
...
5. Assign docs-layer lifecycle states and retention verdicts before moves or
   deletions. If the artifact is inside a Goal Diffusion flow, use
   `$goal-diffusion` for method lifecycle and this skill only for placement,
...
- `signal-placement`: read [Human-Agent SOP](references/human-agent-sop.md),
  ...
  should live. Use `$goal-diffusion` for inbox lifecycle and Goal Pack
  promotion.
...
Goal Diffusion owns Goal Pack charters, state, receipts, inbox lifecycle,
promotion gates, relations, and final audit. This skill only decides where those
artifacts live in `docs/*`, how they relate to SSoT / standards / ADR /
roadmap / specs, and whether cleanup would lose current authority, evidence, or
navigation.

Human escalation is the last resort, not the default response to missing
execution detail. When north star, SSoT / standards, goal objective, claim
boundary, and falsifiable evidence intent are clear, missing harness, missing
...
- Goal contract, principles, boundaries, claim standard, stop rules, evidence
  gates -> `docs/goal-diffusion/goals/**`.
- Implementation spec, task list, checklist, local evidence -> root `specs/**`.
- Weak signal or open candidate -> Goal Diffusion inbox when the project uses
  `$goal-diffusion`; otherwise `docs/proposals/**`.

## Convergence Rules
- `$goal-diffusion` owns method-internal lifecycle: inbox/source/Goal Pack,
  receipt-backed completion, retention/demotion within that method, Goal
  Relations, and final audit. This skill owns docs-layer placement, layer
...
- Do not reimplement `$goal-diffusion` inbox, promotion, retention, demotion, or
...
- continuing would require changing the goal claim boundary, product truth,

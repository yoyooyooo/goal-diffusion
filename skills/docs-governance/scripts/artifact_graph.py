...
DEFAULT_ROOTS = ("docs/goal-diffusion", "docs/roadmap")
...
def audit_findings(nodes: list[Node], edges: list[dict], base_findings: list[dict]) -> list[dict]:
    ...
    for node in nodes:
        ...
        if node.artifact_type == "goal" and node.status in {"ready", "active"}:
            for required in ("objective", "claim_boundary", "evidence_contract"):
        ...
        if node.status == "bridge-needed" and not (as_list(node.fields.get("bridges_to")) or as_list(node.fields.get("next_decision"))):
            findings.append(finding("warn", "GRAPH_BRIDGE_WITHOUT_TARGET", node.path, "bridge-needed node has no bridges_to or next_decision", node_id=node.node_id))
    for edge in edges:
...
def node_to_dict(node: Node) -> dict:
    return {
        ...
        "next_decision": node.fields.get("next_decision", ""),
    }
...
def command_unblock_review(args: argparse.Namespace) -> dict:
    ...
    if node.artifact_type == "goal":
        for field in ("objective", "claim_boundary", "evidence_contract"):
...
def main() -> None:
    ...
    unblock_review.add_argument("--queue", default=QUEUE_DEFAULT, help="optional queue markdown path; omitted by default because Goal Diffusion state is owned by $goal-diffusion")
    ...
    update_node.add_argument("--next-decision")
    update_node.add_argument("--objective")
    update_node.add_argument("--claim-boundary")

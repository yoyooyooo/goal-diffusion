...
- Keep established abbreviations singular by convention: `ssot`, `adr`, and
  `api`.
- Keep method names as named: `goal-diffusion`.
...
|---|---|---|
| `docs/README.md` | Global docs router, layer map, shortest reading paths, conflict order | Domain truth or implementation tasks |
| `docs/product/**` | Product positioning, product philosophy, target users, user value, non-goals, capability meaning | Engineering rules, implementation tasks, current object authority |
| `docs/ssot/**` | Current truths, invariants, domain authority, ownership boundaries | Roadmap sequencing, historical discussion, task lists |
| `docs/standards/**` | Executable rules, commands, quality gates, naming, review gates, SOPs | Product vision or unaccepted proposals |
| `docs/adr/**` | Adopted decisions, rejected alternatives, reasons and consequences | Current task status or full standards |
...
| Layer | Owns | Must not own |
|---|---|---|
| `docs/goal-diffusion/**` | Goal Diffusion method artifacts when the project uses that method: inbox, sources, Goal Packs, receipts, notes, implementation plans when required | Project-wide docs governance, product authority, roadmap truth, or method rules that belong in `$goal-diffusion` |
...
`docs/proposals/**` is optional. If a project uses `$goal-diffusion` as its
...
explicitly needs a non-Goal-Diffusion candidate layer.
...
| `docs/data/**` | Dataset contracts, lineage, semantic metrics, retention, warehouse / analytics definitions | General database implementation tasks or migrations |
...
| `docs/releases/**` | Release notes, changelog policy, rollout records, compatibility notices | Roadmap planning or active implementation tasks |

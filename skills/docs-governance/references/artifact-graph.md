...
Goal Diffusion, report, spec, or roadmap nodes and agents need a small current
...
When a project uses `$goal-diffusion`, the meaning and lifecycle of Goal
Diffusion-specific types are owned by that skill. This graph only indexes
...
claim_boundary: what evidence may prove
evidence_contract: commands or evidence bar
next_decision: nearest unresolved decision
```
...
- scanned roots default to `docs/goal-diffusion` and `docs/roadmap`;
...
python3 scripts/artifact_graph.py update-node \
  --node <node_id> \
  --status ready \
  --next-decision "selected after accepted evidence"
...
For Goal Diffusion inbox/source/Goal Pack lifecycle:

```text
hand off to $goal-diffusion
```
...
If a Goal Diffusion artifact is not ready, route to `$goal-diffusion` instead
...
their evidence reports. Goal Diffusion relations and completion remain owned by
`$goal-diffusion`:
...
them as a scheduler or Goal Diffusion state.
...
frontmatter is correct. Use `review-list` to prioritize advanced agent passes

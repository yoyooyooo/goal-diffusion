...
def _scan_layer_readme_contract(root: Path) -> tuple[list[dict], list[dict]]:
    ...
    for readme in _docs_layer_readmes(root):
        ...
        has_boundary = _has_any(text, LAYER_README_OPTIONAL_SECTIONS["boundary"])
        has_promotion = _has_any(text, LAYER_README_OPTIONAL_SECTIONS["promotion"])
        status.append(
            {
                "path": rel,
                "owns": has_owns,
        ...
        optional_missing = []
        if not has_boundary:
            optional_missing.append("Boundary / Conflict / Priority")
        if not has_promotion:
            optional_missing.append("Promotion / Demotion / lifecycle")
        if optional_missing:
            findings.append(
                {
                    "id": f"DOCS_LAYER_README_AUTHORITY::{rel}",
                    "severity": "warn",
                    "ruleId": "DOCS_LAYER_README_AUTHORITY_SECTIONS_MISSING",
                    "path": str(readme),
                    "summary": f"docs layer README is missing governance sections: {', '.join(optional_missing)}",
                    "evidence": [rel],
                    "fixHint": "add boundary/conflict behavior and promotion/demotion or lifecycle rules",
                }
            )
...
def scan(repo: Path) -> dict:
    ...
    goal_proof_status, goal_proof_missing = _presence(root, GOAL_PROOF_DOCS)
    ...
    if (root / "docs" / "goal-proof").exists():
        for rel in goal_proof_missing:
            findings.append(
                {
                    "id": f"DOCS_GOAL_PROOF::{rel}",
                    "severity": "warn",
                    "ruleId": "DOCS_GOAL_PROOF_INDEX_MISSING",
                    "path": str(root / rel),
                    "summary": f"goal-proof layer is missing expected index: {rel}",
                    "evidence": ["goal-proof layer exists"],
                    "fixHint": "add the missing README or remove the unused goal-proof layer",
                }
            )

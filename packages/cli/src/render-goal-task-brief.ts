...
export function renderBriefText(brief) {
  ...
  lines.push(`objective: ${brief.charter_fields.objective}`);
  lines.push(`authority_refs: ${formatList(brief.charter_fields.authority_refs)}`);
  lines.push(`engineering_guidance.standards: ${formatList(brief.charter_fields.engineering_guidance.standards)}`);
  lines.push(`engineering_guidance.architecture_notes: ${formatList(brief.charter_fields.engineering_guidance.architecture_notes)}`);
  lines.push(`completion.signal: ${brief.charter_fields.completion.signal}`);
  lines.push(`completion.final_proof: ${brief.charter_fields.completion.final_proof}`);
  lines.push(`claim_boundary: ${brief.charter_fields.claim_boundary}`);
  lines.push(`stop_rules: ${formatList(brief.charter_fields.stop_rules)}`);
  lines.push("");

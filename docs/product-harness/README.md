...
- `HarnessEvidence` 引用：receipt、report、test artifact 或外部证据位置。
- claim ceiling、negative claims、not_proven、coverage matrix。
...
- Goal Pack 运行状态、receipt 原文或 final audit 原文。
...
levels:
  ...
  - interface_headless
  - browser_visible
claim_ceiling: browser_visible_candidate
negative_claims:
...
```text
raw execution evidence -> Goal Pack receipts / test artifacts / reports
...
```text
docs/goal-diffusion/goals/<goal-id>/product-harness.yaml
```
